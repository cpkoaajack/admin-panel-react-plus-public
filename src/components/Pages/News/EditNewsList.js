...
import CreateButtonTwoCatalog from '../../CreateButtonTwoCatalog';

class EditNewsList extends Component {
  constructor(props) {
    super(props);
  ...
  configPagination = async () => {
    ...
    try {
      ...
      if (response.success && response.data > 0) {
        ...
        this.setState({
          message: 'No news in the database',
        });
      } else {
        await this.updatePaginationQueryString();
  ...
  render() {
    const { isFetch, itemToFetch, pageCount, news, message } = this.state;
    ...
    const content = (
      <div className='content-main'>
        {news.map((_new) => (
          <NewsItem _new={_new} key={_new._id} />
        ))}
      </div>
    );
        ...
        <div className='local-container'>
          <div className='item-list'>
            <div className='item-list-create'>
              <CreateButtonTwoCatalog
                destination1={`/dashboard/${itemToFetch}/create/activity`}
                destination2={`/dashboard/${itemToFetch}/create/shop`}
              />
            </div>
