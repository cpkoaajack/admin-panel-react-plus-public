...
class EditActivityList extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      /* Indicator */
      isFetch: false,
      itemToFetch: 'activities',
      message: '',

      /* Data - For activity display */
      activities: [],
  ...
  configPagination = async () => {
    ...
    try {
      ...
      if (response.success && response.data > 0) {
        ...
        this.setState({
          message: 'No activity in the database',
        });
      } else {
        await this.updatePaginationQueryString();
  ...
  render() {
    const { isFetch, itemToFetch, pageCount, activities, message } = this.state;
      ...
      return (
        <div className='local-container'>
          <div className='item-list'>
            <div className='item-list-approve'>
              <ApproveButton
                destination={`/dashboard/${itemToFetch}/approve`}
              />
            </div>
